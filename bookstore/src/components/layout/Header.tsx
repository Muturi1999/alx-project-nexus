...
export default function Header() {
  ...
  React.useEffect(() => {
    ...
    // Navigate to books page with search when user searches
    if (debouncedSearchQuery && debouncedSearchQuery !== searchQuery) {
      router.push(`/books?search=${encodeURIComponent(debouncedSearchQuery)}`)
    }
  }, [debouncedSearchQuery, dispatch, router, searchQuery])
